# Adaptado de: https://tlary.github.io/post/fastapi/
...
r = requests.post(url = "http://api:8000/preprocess/", data=dataJSON)
preprocessedData = r.json()
preprocessedJSON = json.dumps(preprocessedData)

# make prediction by making post request to the API
pred = requests.post(url = "http://api:8000/predict/", data=preprocessedJSON)
